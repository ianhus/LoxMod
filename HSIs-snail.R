...
write.table(rbind(cbind(t.sn.6400, r.avg), c(c.avg, "")), file = "Apple_Snail-6400.csv", sep = ",", 
            row.names = c(2005:2015, "AVG"), col.names = c("", 1:ncol(s.6400)))
...
write.table(rbind(cbind(t.sn.3200, r.avg), c(c.avg, "")), file = "Apple_Snail-3200.csv", sep = ",", 
            row.names = c(2005:2015, "AVG"), col.names = c("", 1:ncol(s.3200)))
...
write.table(rbind(cbind(t.sn.1600, r.avg), c(c.avg, "")), file = "Apple_Snail-1600.csv", sep = ",", 
            row.names = c(2005:2015, "AVG"), col.names = c("", 1:ncol(s.1600)))
...
write.table(rbind(cbind(t.sn.800, r.avg), c(c.avg, "")), file = "Apple_Snail-800.csv", sep = ",", 
            row.names = c(2005:2015, "AVG"), col.names = c("", 1:ncol(s.800)))
...
r.avg <- matrix(nrow = 11, ncol = 1); c.avg <- matrix(nrow = 1, ncol = ncol(s.400))

for (r in 1:11) {
  
  r.avg[r,1] <- round(mean(t.sn.400[r,]), 2)
  
}

for (c in 1:ncol(s.400)) {
  
  c.avg[1,c] <- round(mean(t.sn.400[,c]), 2)
  
}


write.table(rbind(cbind(t.sn.400, r.avg), c(c.avg, "")), file = "Apple_Snail-400.csv", sep = ",", 
            row.names = c(2005:2015, "AVG"), col.names = c("", 1:ncol(s.400)))
